    ...
    def add_groupnorm(self):
        ...
        for node in self.graph.nodes:
            if node.op == "Reshape" and node.o().op == "InstanceNormalization" and node.o().o().op == "Reshape" \
                ...
                group_norm = gs.Node("GroupNormalization", "GroupNormalization_{}".format(cnt),
...
class BaseModel():
    ...
    def optimize(self, onnx_graph, return_onnx=True, **kwargs):
        ...
        if kwargs.get('add_groupnorm', False):
            opt.add_groupnorm()
            opt.info(self.name + ': add groupnorm')
...
class UNetXLModel(BaseModel):
    ...
    def optimize(self, onnx_graph):
        if self.unet_gn:
            return super().optimize(onnx_graph, fuse_mha_qkv_int8=True, add_groupnorm=True) 
...
class VAEModel(BaseModel):
    ...
    def get_model(self, torch_inference=''):
        ...
        if not os.path.exists(vae_decoder_model_path):
            ...
            # Modify vae_decoder_model_path
            if self.vae_tiny:
                vae_decoder_model_path = '/workspace/models/sdxl-vae-tiny'
                self.fp16 = True
            elif self.fp16:
                vae_decoder_model_path = '/workspace/models/sdxl-vae-fp16-fix'
            print(f"[I] Load VAE decoder pytorch model from: {vae_decoder_model_path}")
            # Modify torch_dtype
            dtype = torch.float16 if self.fp16 else torch.float32
            model = AutoencoderKL.from_pretrained(vae_decoder_model_path,
    ...
    def optimize(self, onnx_graph):
        if self.vae_gn:
            return super().optimize(onnx_graph, add_groupnorm=True) 
