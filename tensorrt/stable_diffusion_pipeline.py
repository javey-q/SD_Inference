...
class StableDiffusionPipeline:
    ...
    def __init__(
        ...
        def makeScheduler(cls, subfolder="scheduler", **kwargs):
            ...
            return cls.from_pretrained(os.path.join(self.framework_model_dir, self.version, self.pipeline_type.name), subfolder=subfolder)
    ...
    def loadEngines(
        ...
        if 'vae' in self.stages:
            self.models['vae'] = VAEModel(**models_args, fp16=vae_fp16, vae_gn=vae_gn, vae_tiny=vae_tiny)
        ...
        if use_plugin:
            ...
            #     print(c.name)
            # exit()
            plg_registry.get_plugin_creator("GroupNormalizationPlugin", "1", "")
        ...
        for model_name, obj in self.models.items():
            ...
            # add engine for disable_cfg model
            # if model_name.startswith('unet') and self.disable_cfg:
            #     disable_cfg_engine_path = engine_path[model_name] + '_disable_cfg'
            #     disable_cfg_engine = Engine(disable_cfg_engine_path)
            #     if not os.path.exists(disable_cfg_engine_path):
            #         assert extra_build_args is None
            #         disable_cfg_engine.build(onnx_opt_path[model_name],
            #             fp16=fp16amp,
            #             input_profile=obj.get_input_profile(
            #                 opt_batch_size, opt_image_height, opt_image_width,
            #                 static_batch=static_batch, static_shape=static_shape
            ...
            #             enable_all_tactics=enable_all_tactics,
            #             timing_cache=timing_cache,
            #             update_output_names=update_output_names, # TODO
            #             **extra_build_args)
    ...
    def calculateMaxDeviceMemory(self):
        ...
        for model_name, engine in self.engine.items():
            ...
            device_memory_size_gb = engine.engine.device_memory_size / (1024 * 1024 * 1024)  # Convert bytes to gigabytes
            print(model_name, device_memory_size_gb, "GB")
    ...
    def denoise_latent(self,
        ...
        with torch.autocast('cuda', enabled=do_autocast):
            ...
            for step_index, timestep in enumerate(timesteps):
                ...
                # if step_index == int(self.denoising_steps*0.75):
