...
def save_image(image, image_name):
    ...
    images_saved_path = os.path.join(current_dir, '..', 'images_saved', parent_dir_name, sub_dir+'_30_0.7')
...
for generation in queue:
    image = base(generation['prompt'],
        negative_prompt=negative_prompt,
        num_inference_steps=steps,
        output_type='latent').images
    refiner(generation['prompt'], 
        negative_prompt=negative_prompt,
        num_inference_steps=steps,
        image=image)
...
for i, generation in enumerate(queue, start=1):
    ...
    # Create the image
    image = base(prompt=generation['prompt'],
                negative_prompt=negative_prompt,
                height=height,
                width=width,
    ...
    image = refiner(prompt=generation['prompt'],
                negative_prompt=negative_prompt,
                height=height,
                width=width,
                num_images_per_prompt=1,
